...
UTexture2D* FindProjectTextureByNodeName(const FString& TextureName)
{
    ...
    {
        if (AssetData.AssetName.ToString().Equals(TextureName,
...
UImageWidgetBuilder* CreateImageBuilderFromTextureSource(
    const UFigmaNode* OwnerNode,
    const TArray<TObjectPtr<UFigmaNode>>& Children)
{
    for (const UFigmaNode* Child : Children)
    {
        ...
        if (Child->HasProjectTextureReferencePrefix())
        {
            if (UTexture2D* Texture =
                    FindProjectTextureByNodeName(Child->GetNodeName()))
            {
            ...
            UE_LOG_Figma2UMG(
                Warning,
                TEXT("[UFigmaBooleanOp] Node %s references project texture %s, but "
                     "no matching UTexture2D was found."),
                *OwnerNode->GetNodeName(), *Child->GetNodeName());
        }
        else if (Child->HasTextureOnlyImagePrefix())
        {
            if (UTexture2DBuilder* TextureBuilder =
                    GetTextureOnlyImageBuilder(Child))
            {
            ...
            UE_LOG_Figma2UMG(Warning,
                             TEXT("[UFigmaBooleanOp] Node %s uses texture-only child "
                                  "%s, but no texture builder was created."),
                             *OwnerNode->GetNodeName(), *Child->GetNodeName());
        }
    }

    if (OwnerNode && OwnerNode->HasImageWidgetPrefix())
    {
        const FString ImageWidgetTextureName =
            OwnerNode->GetImageWidgetTextureAssetName();
        UImageWidgetBuilder* ImageBuilder = NewObject<UImageWidgetBuilder>();
        ImageBuilder->SetNode(OwnerNode);
        if (UTexture2D* Texture =
                OwnerNode->FindProjectTextureByName(ImageWidgetTextureName))
        {
            ImageBuilder->SetTexture(Texture);
            return ImageBuilder;
        }

        if (!ImageWidgetTextureName.IsEmpty())
        {
            UE_LOG_Figma2UMG(
                Warning,
                TEXT("[UFigmaBooleanOp] IMG_ node %s references project texture %s "
                     "from its own name, but no matching UTexture2D was found."),
                *OwnerNode->GetNodeName(), *ImageWidgetTextureName);
        }
    }
...
bool TryGetVisualTextureSource(const UFigmaNode* OwnerNode,
                               const UFigmaNode* Child,
                               FVisualTextureSource& OutSource)
{
    if (!OwnerNode || !Child)
    {
        return false;
    ...
    if (Child->HasProjectTextureReferencePrefix())
    {
        ...
        UE_LOG_Figma2UMG(Warning,
                         TEXT("[UFigmaBooleanOp] Node %s references project "
                              "texture %s, but no matching UTexture2D was found."),
                         *OwnerNode->GetNodeName(), *Child->GetNodeName());
        return false;
    }
...
bool ApplyVisualTextureFromChildren(
    WidgetBuilderT* WidgetBuilder, const UFigmaNode* OwnerNode,
    const TArray<TObjectPtr<UFigmaNode>>& Children)
{
...
void ApplyCheckBoxVisualTextureSource(UGenericLeafWidgetBuilder* WidgetBuilder,
                                      const FVisualTextureSource& Source,
                                      bool bChecked)
{
    if (!WidgetBuilder)
    {
...
bool ApplyCheckBoxVisualTexturesFromChildren(
    UGenericLeafWidgetBuilder* WidgetBuilder, const UFigmaNode* OwnerNode,
    const TArray<TObjectPtr<UFigmaNode>>& Children)
{
    if (!WidgetBuilder || !OwnerNode)
    {
    ...
    for (int32 Index = 0; Index < Sources.Num(); ++Index)
    {
        const FString SourceName =
            Sources[Index].Node ? Sources[Index].Node->GetNodeName() : FString();
    ...
    ApplyCheckBoxVisualTextureSource(WidgetBuilder, Sources[UncheckedIndex],
                                     false);
    return true;
}
} // namespace

void UFigmaBooleanOp::PostSerialize(const TObjectPtr<UFigmaNode> InParent,
                                    const TSharedRef<FJsonObject> JsonObj)
{
    Super::PostSerialize(InParent, JsonObj);
    SerializeArray(Children, JsonObj, GetJsonArrayName());
}

bool UFigmaBooleanOp::CreateAssetBuilder(
    const FString& InFileKey,
    TArray<TScriptInterface<IAssetBuilder>>& AssetBuilders)
{
    ...
    if (SemanticName.Role == EFigmaUMGWidgetRole::Button ||
        SemanticName.Role == EFigmaUMGWidgetRole::CheckBox)
    {
        return false;
    }
...
TScriptInterface<IWidgetBuilder>
UFigmaBooleanOp::CreateWidgetBuilders(bool IsRoot,
                                      bool AllowFrameButton) const
{
    const FFigmaUMGSemanticName SemanticName = GetUMGSemanticName();
    if (SemanticName.Role == EFigmaUMGWidgetRole::Ignore ||
        HasTextureOnlyImagePrefix() || HasProjectTextureReferencePrefix())
    {
        return nullptr;
    }
    ...
    {
        UGenericLeafWidgetBuilder* CheckBoxBuilder =
            NewObject<UGenericLeafWidgetBuilder>();
    ...
    if (SemanticName.Role == EFigmaUMGWidgetRole::Image ||
        HasImageWidgetPrefix())
    {
        if (UImageWidgetBuilder* ImageBuilder =
                CreateImageBuilderFromTextureSource(this, Children))
        {
